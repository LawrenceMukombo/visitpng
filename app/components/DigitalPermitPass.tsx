...
import {PNG_PERMIT_TYPES, PermitType, IssuedPermit, createPermit} from "../../db/permits";
...
export default function DigitalPermitPass({currency}: DigitalPermitPassProps) {
  const [selectedType, setSelectedType] = useState<PermitType>(PNG_PERMIT_TYPES[0]);
  const [showIssueModal, setShowIssueModal] = useState(false);
  const [holderName, setHolderName] = useState("");
  const [passportOrId, setPassportOrId] = useState("");
  const [countryOfOrigin, setCountryOfOrigin] = useState("Papua New Guinea");
    ...
    <div className="permitsSection">
      <div className="permitsHeader">
        <div>
          ...
          <h2>Trekking Permits & Marine Tags</h2>
          <p>Issue and carry authorized Papua New Guinea park permits and conservation tags with instant offline QR validation.</p>
        </div>
        <button
          type="button"
        ...
        <div className="permitWallet">
          ...
          <div className="permitWalletCards">
            {issuedPermits.map(permit => (
              <article
                key={permit.id}
                className="permitPassCard"
                onClick={() => setActivePermitView(permit)}
              >
                <div className="permitPassTop">
                  <span className="permitAuthorityBadge">{permit.authority}</span>
                  <span className="permitActivePill">● {permit.status.toUpperCase()}</span>
                </div>
                <h4>{permit.permitName}</h4>
                <div className="permitReferenceRow">
                  <strong>Ref: {permit.reference}</strong>
                  <span>Holder: {permit.holderName}</span>
                </div>
                <div className="permitDatesRow">
                  <small>Valid: {permit.startDate} → {permit.expiryDate}</small>
                  <span className="tapToView">Tap to show QR ↗</span>
                </div>
              </article>
            ))}
          </div>
            ...
            <form onSubmit={handleIssuePermit}>
              <label>
                ...
                <select
                  ...
                  onChange={e => {
                    const found = PNG_PERMIT_TYPES.find(x => x.id === e.target.value);
                    if (found) setSelectedType(found);
                  }}
                >
                  {PNG_PERMIT_TYPES.map(t => (
