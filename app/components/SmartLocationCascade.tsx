...
export default function SmartLocationCascade({
    ...
    if (mode === "destination-picker" && selectedDestinationId && destinations.length > 0) {
      ...
      if (selectedProvinceId && provinces.length > 0) {
        ...
        if (prov) {
          ...
          setSelectedRegion((prov.region as PngRegion) || "");
        }
      }
      if (selectedDistrict) {
  ...
  }, [selectedDestinationId, selectedProvinceId, selectedDistrict, destinations, provinces, mode]);
  ...
  const selectedProvName = PNG_PROVINCES.find(p => p.code === selectedProvinceCode)?.name || provinces.find(p => p.code === selectedProvinceCode)?.name;
  ...
  return (
    <div className="smartCascadeContainer">
      <div className="smartCascadeHeader">
        <div className="smartSearchWrapper">
          ...
          <input
            ...
            placeholder="Type anywhere in PNG (e.g. Kokoda, Tufi, Sepik, Rabaul, Kimbe, Goroka...)"
            value={searchQuery}
      ...
      {(selectedRegion || selectedProvinceCode || districtValue || selectedDestObj) && (
        <div className="cascadeBreadcrumb">
          <span className="breadcrumbLabel">Smart Location:</span>
          ...
          {districtValue && <span className="cascadeChip district">{districtValue}</span>}
