...
type Catalogue={categories:Category[];listings:Listing[];meta:{count:number}};
    ...
    {tab==="Explore"?<>
      <section className="hero">
        ...
        <div className="search">
          ...
          <div>
            <span>⌕</span>
            <input id="catalogue-search" value={q} onFocus={()=>setSearchOpen(true)} onBlur={()=>setTimeout(()=>setSearchOpen(false),150)} onChange={e=>{setQ(e.target.value);setSearchOpen(true)}} placeholder="Search places, provinces or experiences" autoComplete="off"/>
          ...
          <nav>
            ...
            <button className={exploreMode==="trails"?"selectedFilter":""} onClick={()=>setExploreMode("trails")}>🗺️ Trails</button>
          </nav>
        </div>
      </section>
      ...
      {exploreMode==="places"?<section className="content">
        ...
        <div className="title">
          <div><p className="eyebrow">PLACES TO EXPLORE</p><h2>{q?`Results for “${q}”`:"Discover something remarkable"}</h2></div>
...
function ProfileScreen({viewer}:{viewer:Viewer}){
  if(!viewer.signedIn)return <section className="accountGuest">
    <div className="accountMark">V</div>
    <p className="eyebrow">YOUR ACCOUNT</p>
    <h1>Your journeys, in one secure place.</h1>
    <p>Sign in to create your traveller profile. Use your Visit PNG email and password to keep your journeys together.</p>
    <a href={viewer.signInPath}>Sign in to Visit PNG</a>
