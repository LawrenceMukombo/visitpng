  ...
  countriesInitPromise = (async () => {
    ...
    await env.DB.prepare(`
      CREATE TABLE IF NOT EXISTS country_settings (
        ...
        map_center_lng REAL NOT NULL DEFAULT 143.955550,
        map_default_zoom INTEGER NOT NULL DEFAULT 6,
        tax_rate REAL NOT NULL DEFAULT 0.10,
        commission_rate REAL NOT NULL DEFAULT 0.05,
        emergency_numbers TEXT NOT NULL DEFAULT '{"ambulance":"111","police":"112","fire":"110","hospital":"+675 7998 8000"}',
        support_email TEXT NOT NULL DEFAULT 'info@visitpng.com',
        support_phone TEXT NOT NULL DEFAULT '+675 321 4188',
    ...
    // 4. Seed Primary Country: Papua New Guinea
    const now = new Date().toISOString();

    const pngExisting = await env.DB.prepare("SELECT id FROM countries WHERE UPPER(code)='PNG' OR UPPER(code)='PG'").first<{ id: number }>();
    ...
    if (!pngId) {
      ...
      `).bind(now, now).run();
      const created = await env.DB.prepare("SELECT id FROM countries WHERE UPPER(code)='PNG'").first<{ id: number }>();
      pngId = created?.id || 1;
    } else {
      await env.DB.prepare(`
        UPDATE countries SET name='Papua New Guinea', currency_code='PGK', currency_symbol='K', phone_code='+675', is_active=1, updated_at=?
        WHERE id=?
      `).bind(now, pngId).run();
    }
    ...
    if (!pngSettingsExisting) {
      await env.DB.prepare(`
        INSERT INTO country_settings (
          ...
          ?, 'visitpng.com', 'Discover the Wonders\nof Papua New Guinea.',
          'Experience the legendary Kokoda Track, vibrant highland sing-sings, pristine coral reefs in Kimbe Bay, and the mystical Sepik River.',
          'LAND OF A MILLION JOURNEYS', '#1B6960', '#E77522', -6.314993, 143.955550, 6,
          0.10, 0.05, '{"ambulance":"111","police":"112","fire":"110","hospital":"+675 7998 8000"}', 'info@visitpng.com', '+675 321 4188',
      ...
      await env.DB.prepare(`
        UPDATE country_settings SET
          domain='visitpng.com', hero_title='Discover the Wonders\nof Papua New Guinea.',
          hero_subtitle='Experience the legendary Kokoda Track, vibrant highland sing-sings, pristine coral reefs in Kimbe Bay, and the mystical Sepik River.',
          hero_eyebrow='LAND OF A MILLION JOURNEYS', brand_color='#1B6960', accent_color='#E77522',
          map_center_lat=-6.314993, map_center_lng=143.955550, map_default_zoom=6,
          tax_rate=0.10, commission_rate=0.05,
          emergency_numbers='{"ambulance":"111","police":"112","fire":"110","hospital":"+675 7998 8000"}',
          support_email='info@visitpng.com', support_phone='+675 321 4188',
          updated_at=?
        WHERE country_id=?
      `).bind(now, pngId).run();
    }
...
export async function getAllCountries(): Promise<CountryWithSettings[]> {
  ...
  const countries = await env.DB.prepare(`
    ...
    FROM countries WHERE is_active=1 ORDER BY CASE WHEN UPPER(code)='PNG' THEN 0 ELSE 1 END, name
  `).all<Country>();
  ...
  if (!country) {
    country = await env.DB.prepare(`
      SELECT id, code, name, currency_code AS currencyCode, currency_symbol AS currencySymbol,
             default_locale AS defaultLocale, phone_code AS phoneCode,
             is_active AS isActive, created_at AS createdAt, updated_at AS updatedAt
      FROM countries WHERE UPPER(code)='PNG' OR UPPER(code)='PG'
    `).first<Country>();
  }
